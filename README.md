...
Three jobs, all green on their first run — see the [Actions tab](https://github.com/rajatslakhina/remote-session-convergence-kit/actions) rather than any run number quoted here, which goes stale on the next commit.

| Job | What it proves |
|---|---|
| `Linux · Swift 6.0` | clean build with `-warnings-as-errors`, then the full suite |
| `macOS · swift test` | the `.macOS(.v14)` platform declaration, and the SwiftUI console compiling against a real SDK |
| `iOS Simulator · build` (demo repo) | that Xcode genuinely resolves this package **from GitHub at `1.0.0`** and compiles an app against it |

The third is the one worth caring about: it is the only thing that exercises the remote package reference rather than a local checkout.

### What is still not verified

- **Interactive behaviour driven by hand on a device.** Clicks into the Simulator window were blocked by the host environment during the run, so only the default state was exercised manually. The strategy toggle and the three-round degrade sequence are covered by passing tests, not by tapping.
- **32-bit `Int` behaviour**, as noted above — reasoning, not a measurement.
